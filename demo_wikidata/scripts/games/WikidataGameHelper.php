...
class WikidataGameHelper {
	...
	function update_no_image () {
		foreach ( $this->update_no_image_generator() AS $chunk ) {
			...
			# Has an image
			if ( count($chunk) == 0 ) continue ;
			$sql = "SELECT DISTINCT `page_title` FROM `page`,`pagelinks` WHERE `page_namespace`=0 AND `page_id`=`pl_from` AND `pl_namespace`=120 AND `pl_title`='P18' AND `page_title` IN ('".implode("','",array_keys($chunk))."')" ;
			$result = $this->tfc->getSQL ( $this->dbwd , $sql ) ;
			$to_update = [] ;
