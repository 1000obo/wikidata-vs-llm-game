...
function checkCandidates ( $site , $candidate_pages ) {
	...
	$sql = "select ips_item_id FROM wb_items_per_site WHERE ips_site_id='$site' AND ips_site_page IN ('" . implode ( "','" , $candidate_pages ) . "') AND NOT EXISTS (SELECT * FROM pagelinks,wb_entity_per_page WHERE ips_item_id=epp_entity_id and pl_from=epp_page_id and pl_namespace=120 and pl_title='P31')" ;
...
$sql = "select DISTINCT term_entity_id from wb_terms where term_entity_type='item' and term_type='description' and term_text='Wikipedia disambiguation page' and not exists (SELECT * FROM pagelinks,wb_entity_per_page WHERE term_entity_id=epp_entity_id and pl_from=epp_page_id and pl_namespace=120 and pl_title='P31')" ;
...
/* MAYBE LATER
// Items without "instance of", having "(" in label
$sql = "select DISTINCT term_entity_id from wb_terms where term_entity_type='item' and term_type='label' and term_text LIKE '%(%' and not exists (SELECT * FROM pagelinks,wb_entity_per_page WHERE term_entity_id=epp_entity_id and pl_from=epp_page_id and pl_namespace=120 and pl_title='P31')"
*/
