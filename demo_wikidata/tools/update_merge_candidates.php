...
while( $line = fgets( $f ) ) {
	...
	$sql = "SELECT DISTINCT REPLACE(page_title, 'Q', '') AS entity_id FROM page,pagelinks WHERE page_title IN ('" . implode("','Q",$all_items) . "')" ;
	$sql .= " AND page_namespace = 0 AND page_id=pl_from AND pl_namespace=0 AND pl_title IN ($wp_types)" ;
//	print "$sql\n" ;
