...
foreach ( $people AS $site => $list ) {
	...
	foreach ( $list AS $q => $title ) {
		$sql = "SELECT * FROM page,pagelinks WHERE page_title=\"$title\" and page_namespace=0 and page_id=pl_from and pl_namespace=0 and pl_title IN ($occs)" ;
		if(!$result = $dbw->query($sql)) die('There was an error running the query 3[' . $db->error . '] '."$site : $sql");
		while($o = $result->fetch_object()){
			if ( !isset ( $occ_rev[$site][$o->pl_title] ) ) { print "Dunno $site:".$o->pl_title."\n"; continue ; }
			$person2occ[$q][$occ_rev[$site][$o->pl_title]] = 1 ;
		}
	}
}
