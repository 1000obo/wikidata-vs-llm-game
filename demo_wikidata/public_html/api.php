...
function doesItemHaveLink ( $item , $target ) {
	...
	$sql = "select * from page,pagelinks where page_title='Q$item' AND page_namespace=0 AND page_id=pl_from AND pl_namespace=$ns AND pl_title='$target' LIMIT 1" ;
...
//		if ( !checkSettingLanguages($x->item1) ) return false ;
//		if ( !checkSettingLanguages($x->item2) ) return false ;
